...
const NFT_ITEM_NEW_OWNER_ADDRESS = ""
...
func main() {
	...
	internalMessage := func() *cell.Cell {
		msg := tlb.InternalMessage{
			DstAddr: address.MustParseAddr(NFT_ITEM_ADDRESS),
			Amount:  tlb.MustFromTON("0.15"),
			Body: cell.BeginCell().
				MustStoreUInt(0x5fcc3d14, 32).
				MustStoreUInt(0, 64).
				MustStoreAddr(address.MustParseAddr(NFT_ITEM_NEW_OWNER_ADDRESS)).
...

	{
		...
		owner := parsedResBody["result"].([]any)[0].(map[string]any)["owner_address"].(string)

		fmt.Println(
			"\nThe old NFT Item owner is",
			address.MustParseRawAddr(owner).Bounce(true).String(),
		)
	}
	...
	fmt.Println("Wait for the NFT Item transfer")
